...
define(function (require) {
    ...
    BMapExt.prototype._init = function (obj, BMap, ec, mapOption) {
                    ...
                    for (var i = 0, item; item = series[i++];) {
                        ...
                        if (data && data.length) {
                            data.forEach(function(o,k){
                                var pos = self.geoCoord2Pixel([self._geo[k][0],self._geo[k][1]]);
                                o[0]= pos[0] - self._mapOffset[0];
                                o[1]= pos[1] - self._mapOffset[1];
                            });
                        }
                    }
                    self._ec._setOption(option, true,true);
            }
        ...
        self.setTimeLineOption = function(){
            var timeline = self._ec.component.timeline;
            timeline.options.forEach(function(option){
                    var series = option.series || {};
        ...
        self.refresh = function () {
            if (self._ec) {
                ...
                if(self._ec.component.timeline)
                self.setTimeLineOption();
            }
        }
